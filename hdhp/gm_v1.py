...
class HDHProcess:
    ...
    def __init__(self, num_cluster, num_global_patterns, num_patterns_per_cluster, num_patterns, alpha_0, mu_0, beta_0, zeta_0,vocabulary, omega=1,
        ...
        beta_0 : tuple
            The parameter of the Beta distribution that is used to sample
            each cluster's \psi (global-local weight). This tuple should be of the
            form (shape, scale).

        zeta_0 : tuple
            The parameter of the dirichlet distribution that is used to sample
            each user's cluster label. This tuple should be of the
            form ([num_cluster]]).
        ...
        self.cluster_prng = RandomState(self.prng.randint(2000000000))

        self.num_cluster = num_cluster
        self.num_patterns_per_cluster = num_patterns_per_cluster
        self.num_global_patterns = num_global_patterns
        ...
        self.global_pattern_params, self.local_pattern_params = self.sample_pattern_params() # get the word distribution
        self.global_time_kernels, self.local_time_kernels = self.sample_time_kernels() # get the kernel value 
        self.global_pattern_popularity, self.local_pattern_popularity = self.sample_pattern_popularity() # get the pattern popularity
        self.cluster_weights = self.sample_cluster_hot()
        self.cluster_multi = self.prng.dirichlet(self.zeta_0)
    ...
    def reset(self):
        ...
        """
        self.mu_per_user = {}
        self.cluster_per_user = {}
        self.num_users = 0
        self.time_history = []
        self.time_history_per_user = {}
        self.table_history_per_user = {}
        self.dish_on_global_table_per_user = {}
        self.dish_on_local_table_per_user = {}
        self.global_dish_counters = defaultdict(int)
        self.local_dish_counters = {}
        self.last_event_user_global_pattern = defaultdict(lambda: defaultdict(int))
        self.last_event_user_local_pattern = defaultdict(lambda: defaultdict(int))
        self.total_global_tables = 0
        self.total_local_tables = {}
        self.global_first_observed_time = {}
        self.local_first_observed_time = {}
        self.user_global_table_cache = defaultdict(dict)
        self.user_local_table_cache = defaultdict(dict)
        ...
        self.level_history_per_user = defaultdict(list)
        self.dish_on_global_table_per_user = defaultdict(dict)
        self.dish_on_local_table_per_user = defaultdict(dict)
        self.cache_per_user = defaultdict(dict)
        self.total_global_tables_per_user = defaultdict(int)
        self.total_local_tables_per_user = defaultdict(int)
        self.events = []
        self.per_global_pattern_word_counts = defaultdict(lambda: defaultdict(int))
        self.per_global_pattern_word_count_total = defaultdict(int)
        self.per_local_pattern_word_counts = {}
        self.per_local_pattern_word_count_total = {}

        for cluster in range(self.num_cluster):
            self.total_local_tables[cluster] = 0
            self.per_local_pattern_word_counts[cluster] = defaultdict(lambda: defaultdict(int))
            self.per_local_pattern_word_count_total[cluster] = defaultdict(int)
            self.local_first_observed_time[cluster] = {}
            self.local_dish_counters[cluster] = defaultdict(int)
    ...
    def sample_pattern_params(self):
        ...
        local_sampled_params = {}
        V = len(self.vocabulary)
        for pattern in range(self.num_global_patterns):
            custom_theta = [0.000000001] * V
            ...
            for word in words_in_pattern:
                ...
                global_sampled_params[pattern] = \
                    self.pattern_param_prng.dirichlet(custom_theta)
        for cluster in range(self.num_cluster):
            sampled_params_per_cluster = {}
            for pattern in range (self.num_patterns_per_cluster[cluster]):
                custom_theta = [0.000000001] * V
                words_in_pattern = self.prng.choice(V, size=self.words_per_pattern,
                                                replace=False)
                for word in words_in_pattern:
                    custom_theta[word] = 100. / self.words_per_pattern
                    sampled_params_per_cluster[pattern] = \
                        self.pattern_param_prng.dirichlet(custom_theta)
            local_sampled_params[cluster] = sampled_params_per_cluster


        return global_sampled_params, local_sampled_params
    ...
    def sample_time_kernels(self):
        ...
        """
        global_alphas = {pattern: self.time_kernel_prng.gamma(self.alpha_0[0],
                                                       self.alpha_0[1])
                  for pattern in range(self.num_global_patterns)}
        local_alphas = {}
        for cluster in range(self.num_cluster):
            tmp_alphas = {pattern: self.time_kernel_prng.gamma(self.alpha_0[0],
                                                       self.alpha_0[1])
                  for pattern in range(self.num_patterns_per_cluster[cluster])}
            local_alphas[cluster] = tmp_alphas

        return global_alphas, local_alphas
    ...
    def sample_pattern_popularity(self):
        ...
        """
        global_pattern_popularity = {}
        local_pattern_popularity = {} 
        pi = self.pattern_param_prng.dirichlet([1 for pattern
                                                in range(self.num_global_patterns)])
        for pattern_i, pi_i in enumerate(pi):
            global_pattern_popularity[pattern_i] = pi_i

        for cluster in range(self.num_cluster):
            temp_pattern_popularity = {} 
            pi = self.pattern_param_prng.dirichlet([1 for pattern
                                                in range(self.num_patterns_per_cluster[cluster])])
            for pattern_i, pi_i in enumerate(pi):
                temp_pattern_popularity[pattern_i] = pi_i
            local_pattern_popularity[cluster] = temp_pattern_popularity
        return global_pattern_popularity, local_pattern_popularity

    def sample_cluster(self):
        print(self.cluster_multi)
        label = self.prng.multinomial(1,self.cluster_multi).nonzero()
        print(label)
        return label[0][0]

    def sample_cluster_hot(self):
        cluster_weights = {}
        for cluster in range(self.num_cluster):
            cluster_weights[cluster] = self.cluster_prng.beta(self.beta_0[0], self.beta_0[1])      
    ...
    def sample_next_time(self, is_global, pattern, user):
        ...
        cluster = self.cluster_per_user[user]
        if is_global is True:
            lambda_u_pattern = mu_u * self.cluster_weights[cluster] * self.global_pattern_popularity[pattern]
        else:
            lambda_u_pattern = mu_u * (1-self.cluster_weights[cluster]) * self.local_pattern_popularity[cluster][pattern]

        if is_global is True:
            if user in self.total_global_tables_per_user:
                # We have seen the user before
                num_tables = self.total_global_tables_per_user[user]
                pattern_tables = [table for table in range(num_tables)
                                  if self.dish_on_global_table_per_user[user][table]
                                  == pattern]
            else:
                pattern_tables = []
        else:
            if user in self.total_local_tables_per_user:
                # We have seen the user before
                num_tables = self.total_local_tables_per_user[user]
                pattern_tables = [table for table in range(num_tables)
                                  if self.dish_on_local_table_per_user[user][table]
                                  == pattern]
            else:
                pattern_tables = []
        if is_global is True:
            alpha = self.global_time_kernels[pattern]
        else:
            alpha = self.local_time_kernels[cluster][pattern]
        if not pattern_tables:
            ...
            if is_global is True:
                s = self.last_event_user_global_pattern[user][pattern]
            else:
                s = self.last_event_user_local_pattern[user][pattern]
            pattern_intensity = 0
            for table in pattern_tables:
                if is_global is True:
                    t_last, sum_kernels = self.user_global_table_cache[user][table]
                else:
                    t_last, sum_kernels = self.user_local_table_cache[user][table]
    ...
    def find_next_event_over_all(self, next_time_per_global_pattern ,next_time_per_local_pattern):
        z_n_global = next_time_per_global_pattern.argmin()
        t_n_global = next_time_per_global_pattern[z_n_global]

        z_n_local = next_time_per_local_pattern.argmin()
        t_n_local = next_time_per_local_pattern[z_n_local]

        if t_n_global > t_n_local:
            return False, t_n_local, z_n_local
        else:
            return True, t_n_global, z_n_global
    ...
    def sample_user_events(self, min_num_events=100, max_num_events=None,
        ...
        self.cluster_per_user[user] = cluster

        # Populate the list with the first event for each pattern
        next_time_per_global_pattern = [self.sample_next_time(True, pattern, user)
                                 for pattern in range(self.num_global_patterns)]
        next_time_per_global_pattern = asfortranarray(next_time_per_global_pattern)

        next_time_per_local_pattern = [self.sample_next_time(False, pattern, user)
                                 for pattern in range(self.num_patterns_per_cluster[cluster])]
        next_time_per_local_pattern = asfortranarray(next_time_per_local_pattern)
        ...
        while iteration < min_num_events or not over_tmax:
            ...
            is_global, t_n, z_n = self.find_next_event_over_all(next_time_per_global_pattern,next_time_per_local_pattern)

            if t_max is not None and t_n > t_max:
                over_tmax = True
                break
            if is_global is True:
                num_tables_user = self.total_global_tables_per_user[user] \
                    if user in self.total_global_tables_per_user else 0
                tables = range(num_tables_user)
                tables = [table for table in tables
                          if self.dish_on_global_table_per_user[user][table] == z_n]
                intensities = []

                alpha = self.global_time_kernels[z_n]
                for table in tables:
                    t_last, sum_kernels = self.user_global_table_cache[user][table]
                ...
                intensities.append(mu_u * self.cluster_weights[cluster] * self.global_pattern_popularity[z_n])
                ...
                if k < len(tables):
                    ...
                    self.user_global_table_cache[user][table] = (t_n, sum_kernels)
                else:
                    table = num_tables_user
                    self.total_global_tables += 1
                    self.total_global_tables_per_user[user] += 1
                    # Since this is a new table, initialize the cache accordingly
                    self.user_global_table_cache[user][table] = (t_n, 0)
                    self.dish_on_global_table_per_user[user][table] = z_n

                if z_n not in self.global_first_observed_time or\
                        t_n < self.global_first_observed_time[z_n]:
                    self.global_first_observed_time[z_n] = t_n
                self.global_dish_counters[z_n] += 1

                doc_n = self.sample_document(z_n,True)
                ...
                self.level_history_per_user[user].append(True)
                self.last_event_user_global_pattern[user][z_n] = t_n

                # Resample time for that pattern
                next_time_per_global_pattern[z_n] = self.sample_next_time(True, z_n, user)              
            else:
                num_tables_user = self.total_local_tables_per_user[user] \
                    if user in self.total_local_tables_per_user else 0
                tables = range(num_tables_user)
                tables = [table for table in tables
                          if self.dish_on_local_table_per_user[user][table] == z_n]
                intensities = []

                alpha = self.local_time_kernels[cluster][z_n]
                for table in tables:
                    t_last, sum_kernels = self.user_local_table_cache[user][table]
                    update_value = self.kernel(t_n, t_last)
                    table_intensity = alpha * sum_kernels * update_value
                    table_intensity += alpha * update_value
                    intensities.append(table_intensity)
                intensities.append(mu_u * (1-self.cluster_weights[cluster]) * self.local_pattern_popularity[cluster][z_n])
                log_intensities = [ln(inten_i)
                                   if inten_i > 0 else -float('inf')
                                   for inten_i in intensities]
                ...
                if k < len(tables):
                    ...
                    sum_kernels += 1
                    sum_kernels *= update_value
                    self.user_local_table_cache[user][table] = (t_n, sum_kernels)
                else:
                    table = num_tables_user
                    self.total_local_tables[cluster] += 1
                    self.total_local_tables_per_user[user] += 1
                    # Since this is a new table, initialize the cache accordingly
                    self.user_local_table_cache[user][table] = (t_n, 0)
                    self.dish_on_local_table_per_user[user][table] = z_n

                if z_n not in self.local_first_observed_time[cluster] or\
                        t_n < self.local_first_observed_time[cluster][z_n]:
                    self.local_first_observed_time[cluster][z_n] = t_n
                self.local_dish_counters[cluster][z_n] += 1

                doc_n = self.sample_document(z_n, False, cluster)
                self._update_word_counters(doc_n.split(), z_n, False, cluster)
                self.document_history_per_user[user]\
                    .append(doc_n)
                self.table_history_per_user[user].append(table)
                self.time_history_per_user[user].append(t_n)
                self.level_history_per_user[user].append(False)
                self.last_event_user_local_pattern[user][z_n] = t_n
                # Resample time for that pattern
                next_time_per_local_pattern[z_n] = self.sample_next_time(False, z_n, user) 
            iteration += 1
        ...
        print(cluster)
        print(len(events))
        print(self.level_history_per_user[user].count(True))
        # print(self.global_pattern_popularity)
        print(self.dish_on_global_table_per_user[user])
        # print(self.local_pattern_popularity[cluster])
    ...
    def sample_document(self, pattern, is_global, cluster=None):
        ...
        if is_global is True:
            words = self.doc_prng.multinomial(length, self.global_pattern_params[pattern])
        else:
            words = self.doc_prng.multinomial(length, self.local_pattern_params[cluster][pattern])

        return ' '.join([self.vocabulary[i]
                         for i, repeats in enumerate(words)
                         for j in range(repeats)])

    def _update_word_counters(self, doc, pattern, is_global=True, cluster=None):
        ...
        if is_global is True:
            for word in doc:
                self.per_global_pattern_word_counts[pattern][word] += 1
                self.per_global_pattern_word_count_total[pattern] += 1
        else:
            for word in doc:
                self.per_local_pattern_word_counts[cluster][pattern][word] += 1
                self.per_local_pattern_word_count_total[cluster][pattern] += 1
        return
